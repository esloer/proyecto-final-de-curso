document.addEventListener('DOMContentLoaded', function() {
    const inputSeries = document.getElementById('bus-peliculas');
    ...
    function buscarSeries(term) {
        ...
        xhr.open('GET', `/buscarPeliculasNetflix?titulo=${term}`, true);
    ...
    function mostrarTodasLasSeries() {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('GET', '/buscarPeliculasNetflix?titulo=', true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    const responseData = JSON.parse(xhr.responseText);
